            ...
            else
            {
                ...
                if(user)
                {
                    ...
                    if(r == 0)
                    {
                        ...
                        user_context *user = &client_list[index];

                        if(user->username[0] == '\0')
                        {
                            printf("[%d] failed to log in",user->user_fd);
                        }
                        else {
                            char gone_msg[MAX_MESSAGE_SIZE];
                            snprintf(gone_msg, MAX_MESSAGE_SIZE, "User [%s] is gone!\n",user->username);

                            for(int k = 0; k < current_connections_number; k++) {
                                if(client_list[k].user_fd != user->user_fd && client_list[k].username[0] != '\0') {
                                    send_colored_message(client_list[k].user_fd, SOP_RED, gone_msg);
                                }
                            }
                            printf("[%d] known as [%s] has disconnected\n", user->user_fd, user->username);
                        }

                        if(epoll_ctl(epoll_ds, EPOLL_CTL_DEL, client_fd, NULL) == -1) {
                        ...
                        if(index != current_connections_number -1)
                        {
                            client_list[index] = client_list[current_connections_number-1];
                        }
                        current_connections_number--;
                        continue;
