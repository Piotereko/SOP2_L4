...
void doClient(int fd, struct sockaddr_in addr, int file)
{
    ...
    do
    {
        ...
        *((int32_t *)buf) = htonl(++chunkNo);//on the firt place in buf number of chanks

        if (size < MAXBUF - offset)
        {
            last = 1;
            memset(buf + offset + size, 0, MAXBUF - offset - size);//set to 0 from size + oofset to the end of buf
        }

        *(((int32_t *)buf) + 1) = htonl(last); //info if its the last package
